...
// Shows slots stats per offer + blocks accepting when slots are full.
...
function SlotsBar({ slots }) {
  if (!slots) return null;
  const { total, accepted, remaining, isFull } = slots;
  const pct = total > 0 ? Math.round((accepted / total) * 100) : 0;

  return (
    <div className="slots-bar-wrap">
      <div className="slots-bar-header">
        <span className="slots-bar-label">
          <UsersIcon /> Available Slots
        </span>
        <span className={`slots-bar-count ${isFull ? 'slots-bar-count--full' : ''}`}>
          {isFull
            ? '🔴 All slots filled — offer closed'
      ...
      {/* Counters row */}
      <div className="slots-bar-stats">
        <span className="slots-bar-stat">
          <span className="slots-bar-stat__dot" style={{ background: '#94a3b8' }} />
          Total: {total}
        </span>
        <span className="slots-bar-stat">
          <span className="slots-bar-stat__dot" style={{ background: '#10b981' }} />
          Accepted: {accepted}
        </span>
        <span className="slots-bar-stat">
          <span className="slots-bar-stat__dot" style={{ background: isFull ? '#ef4444' : '#7c3aed' }} />
          Remaining: {remaining}
        </span>
      </div>
    </div>
...
export default function CompanyCandidates() {
  ...
  const [slots, setSlots]               = useState(null);   // ← slot stats
  const [filterOffer, setFilterOffer]   = useState(paramOfferId || 'all');
  ...
  // ── Load candidates for selected offer (or all offers) ──
  useEffect(() => {
    const fetchCandidates = async () => {
      ...
      try {
        if (filterOffer !== 'all') {
          // Single offer → get slot stats too
          const res = await api.get(`/company/candidates/${filterOffer}`);
          setCandidates((res.data.data || []).map(a => ({
            ...a,
            offerTitle: offers.find(o => o._id === filterOffer)?.title || '',
            offerId:    filterOffer,
          })));
          setSlots(res.data.slots || null);   // ← store slot stats
        } else {
          // All offers → merge, no per-offer slot stats
          const fetchedOffers = offers.length > 0 ? offers : (await api.get('/company/offers')).data.data || [];
  ...
  const handleAction = async (appId, action) => {
    ...
    setActionError('');
    try {
      const endpoint = `/company/candidates/${appId}/${action}`;
      ...
      // Update slot stats from response
      if (res.data.slots) setSlots(res.data.slots);

      setConfirmAction(null);
    } catch (err) {
      // Show the backend error message (e.g. "No slots remaining...")
  ...
  const stats = {
    ...
    accepted: candidates.filter(c => c.status === 'ACCEPTED' || c.status === 'VALIDATED').length,
  ...
  return (
    <div className="dashboard">
      ...
      <main className="dashboard-main">
        ...
        <div className="candidates-filters">
          <div className="applications-filters applications-filters--no-margin">
            {['all','PENDING','ACCEPTED','REFUSED'].map(s => (
              <button key={s} className={`app-filter ${filterStatus === s ? 'app-filter--active' : ''}`}
          ...
          {offers.length > 0 && (
            <select className="input candidates-filter__select"
              value={filterOffer}
              onChange={e => setFilterOffer(e.target.value)}
              style={{ width: 'auto', background: '#fff', color: '#0f0f1a', border: '1.5px solid #e2e8f0', borderRadius: 10, padding: '8px 14px', fontSize: 13, fontFamily: 'inherit' }}>
        ...
        {/* ── Slots bar — shown only when a specific offer is selected ── */}
        {filterOffer !== 'all' && <SlotsBar slots={slots} />}

        {/* Error message from backend (e.g. slots full) */}
        {actionError && (
          <div className="slots-error-banner">
            <svg width="16" height="16" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24">
              <circle cx="12" cy="12" r="10"/><path d="M12 8v4M12 16h.01"/>
            </svg>
            {actionError}
            <button className="slots-error-close" onClick={() => setActionError('')}>✕</button>
          </div>
        )}
        ...
        <div className="applications-list">
          ...
          : filtered.length === 0 ? (
            ...
            // Disable accept button if slots are full and this candidate is still pending
            const slotsFull = slots?.isFull && candidate.status === 'PENDING';
            ...
            return (
              <div key={candidate._id} className="application-card">
                  ...
                  <div className="application-card__company">
                    ...
                    <div className="application-card__info">
                      ...
                      <p className="application-card__company-name">
                        {student.level} · {student.specialty} · {student.university || ''}
                      </p>
                    </div>
                  </div>
                  <span className="candidate-status-badge"
                    style={{ background: s.bg, color: s.color, borderRadius: 8, padding: '4px 14px', fontSize: 12, fontWeight: 700, flexShrink: 0 }}>
                    {s.label}
                  </span>
                </div>
                ...
                <div style={{ margin: '8px 0' }}>
                  <span style={{ background: 'rgba(124,58,237,.1)', color: '#a78bfa', borderRadius: 8, padding: '3px 12px', fontSize: 12, fontWeight: 600 }}>
                ...
                <div className="application-card__meta">
                  ...
                  {/* Show slot warning on pending cards when offer is full */}
                  {slotsFull && (
                    <span style={{ fontSize: 12, color: '#ef4444', fontWeight: 600, marginLeft: 12 }}>
                      🔴 No slots remaining
                    </span>
                  )}
                </div>
                ...
                <div className="application-card__actions">
                  ...
                  {candidate.status === 'PENDING' && (
                    <>
                      {/* Accept button — disabled if slots full */}
                      <button
                        className={`btn--accept ${slotsFull ? 'btn--accept--disabled' : ''}`}
                        disabled={slotsFull}
                        title={slotsFull ? 'No slots remaining for this offer' : 'Accept this candidate'}
                        onClick={() => !slotsFull && setConfirmAction({
                          id: candidate._id, action: 'accept',
                          name: `${student.firstName} ${student.lastName}`,
                        })}
                        style={{ opacity: slotsFull ? 0.4 : 1, cursor: slotsFull ? 'not-allowed' : 'pointer' }}
                      >
                        <CheckIcon /> Accept
                        {slots && !slotsFull && (
                          <span style={{ fontSize: 11, marginLeft: 4, opacity: 0.8 }}>
                            ({slots.remaining} left)
                          </span>
                        )}
                      </button>
                      ...
                      <button className="btn--refuse"
                        onClick={() => setConfirmAction({
                          id: candidate._id, action: 'refuse',
                          name: `${student.firstName} ${student.lastName}`,
                        })}>
          ...
          <div className="modal-box modal-box--large">
            <div className="cv-modal__header">
              <div className="cv-modal__avatar">
                {(selectedCandidate.studentId?.firstName || 'S').charAt(0)}
              </div>
              <div className="cv-modal__info">
                <div className="cv-modal__name">
                  {selectedCandidate.studentId?.firstName} {selectedCandidate.studentId?.lastName}
                </div>
                <div className="cv-modal__level">
                  {selectedCandidate.studentId?.level} · {selectedCandidate.studentId?.specialty}
                </div>
                <div className="cv-modal__university">
                  {selectedCandidate.studentId?.university || ''}
                </div>
              </div>
              <span style={{
                background: statusConfig[selectedCandidate.status]?.bg,
                color:      statusConfig[selectedCandidate.status]?.color,
            ...
            {[
              ['Bio',    selectedCandidate.studentId?.bio],
              ['Wilaya', selectedCandidate.studentId?.wilaya],
              ['Phone',  selectedCandidate.studentId?.phone],
            ...
            <div className="cv-modal__skills-section">
              ...
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8, marginTop: 8 }}>
                {(selectedCandidate.studentId?.skills || []).map(sk => (
            ...
            {selectedCandidate.studentId?.githubLink && (
              <a className="cv-modal__github-link"
                href={`https://${selectedCandidate.studentId.githubLink}`}
            ...
            <div className="cv-modal__actions">
              {selectedCandidate.status === 'PENDING' && (
                <>
                  <button
                    className="btn--cv-accept"
                    disabled={slots?.isFull}
                    style={{ opacity: slots?.isFull ? 0.4 : 1, cursor: slots?.isFull ? 'not-allowed' : 'pointer', background: '#10b981' }}
                    title={slots?.isFull ? 'No slots remaining' : ''}
                    onClick={() => {
                      if (slots?.isFull) return;
                      setSelected(null);
                      setConfirmAction({
                        id: selectedCandidate._id, action: 'accept',
                        name: `${selectedCandidate.studentId?.firstName}`,
                      });
                    }}
                  >
                    <CheckIcon />
                    {slots?.isFull ? 'No Slots Left' : 'Accept'}
                  </button>
                  <button className="btn--cv-refuse"
                    onClick={() => {
                      setSelected(null);
                      setConfirmAction({ id: selectedCandidate._id, action: 'refuse', name: `${selectedCandidate.studentId?.firstName}` });
                    }}>
                    Refuse
                  </button>
                </>
        ...
        <div className="modal-overlay">
          <div className="modal-box">
            <h3 className="modal-box__title">
              {confirmAction.action === 'accept' ? 'Accept Candidate?' : 'Refuse Candidate?'}
            </h3>

            {/* Show remaining slots in accept modal */}
            {confirmAction.action === 'accept' && slots && (
              <div className="slots-confirm-info">
                <div className="slots-confirm-row">
                  <span>Total slots</span><strong>{slots.total}</strong>
                </div>
                <div className="slots-confirm-row">
                  <span>Already accepted</span><strong style={{ color: '#10b981' }}>{slots.accepted}</strong>
                </div>
                <div className="slots-confirm-row">
                  <span>Remaining after this</span>
                  <strong style={{ color: slots.remaining - 1 === 0 ? '#ef4444' : '#7c3aed' }}>
                    {slots.remaining - 1}
                    {slots.remaining - 1 === 0 && ' — offer will be closed'}
                  </strong>
                </div>
              </div>
            )}
