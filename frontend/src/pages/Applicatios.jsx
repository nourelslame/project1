...
// My Applications page — shows all applications with status.
// For VALIDATED applications, shows a "Download Agreement" button.
...
function ApplicationCard({ application, onCancel, onDownload }) {
  ...
  return (
    <div className="application-card">
      {/* Header: company logo + title + status badge */}
      <div className="application-card__header">
        <div className="application-card__company">
          <div className="application-card__logo">{(company.name || 'C').charAt(0)}</div>
      ...
      <div className="application-card__details">
        ...
        <span className="application-card__detail">{offer.duration ? `${offer.duration} months` : ''}</span>
      </div>
      ...
      <div className="application-card__actions">
        ...
        {/* ── VALIDATED: show agreement download + preview ── */}
          ...
          <div className="agreement-actions">
            {application.hasPdf ? (
              <>
                ...
                <button
                  className="btn--agreement-download"
                  onClick={() => onDownload(application.agreementId, 'download')}
                >
                  <DownloadIcon /> Download Agreement (PDF)
                </button>
                {/* View agreement info in modal */}
                <button
                  className="btn--agreement-preview"
                  onClick={() => onDownload(application.agreementId, 'preview')}
                >
              ...
              <span className="agreement-pending-text">
                ⏳ Agreement PDF is being generated…
              </span>
            )}
          </div>
        )}

        {/* ACCEPTED but not yet validated by admin */}
      ...
      <div className="modal-box modal-box--large">
        ...
        <div className="doc-preview">
          <div className="doc-preview__header">
            <div className="doc-preview__uni">UNIVERSITÉ — FERHAT ABBAS SÉTIF 1</div>
          ...
          <div className="doc-preview__body">
            {[
              ...
              ['University',   agreement.universityName || ''],
              ['Host Company', company.name || ''],
              ['Supervisor',   agreement.supervisorName || ''],
              ['Start Date',   agreement.startDate ? new Date(agreement.startDate).toLocaleDateString('en-GB', { day: 'numeric', month: 'long', year: 'numeric' }) : ''],
              ['End Date',     agreement.endDate   ? new Date(agreement.endDate).toLocaleDateString('en-GB',   { day: 'numeric', month: 'long', year: 'numeric' }) : ''],
              ['Generated',    agreement.createdAt ? new Date(agreement.createdAt).toLocaleDateString() : ''],
            ].map(([label, value]) => (
...
export default function MyApplications() {
  ...
  const [previewAgreement, setPreviewAgreement] = useState(null);  // agreement object for modal
  const [previewLoading, setPreviewLoading]     = useState(false);

  // ── Fetch all applications (with agreement info for VALIDATED ones) ──
  useEffect(() => {
  ...
  // ── Download or preview an agreement ──
  const handleAgreement = async (agreementId, mode) => {
    if (!agreementId) { alert('Agreement ID not found.'); return; }

    if (mode === 'download') {
      // Download: open in new tab — browser triggers file download
      // We need to pass the token since the route is protected.
      // Simplest approach: fetch as blob then create object URL
      try {
        const res = await api.get(
          `/student/agreements/${agreementId}/download`,
          { responseType: 'blob' }
        );
      ...
      // Preview: load agreement data and show in modal
      ...
      try {
        ...
        else alert('Agreement details not found.');
      } catch (err) {
        alert(err.response?.data?.message || 'Failed to load agreement details.');
      } finally {
      ...
      <main className="dashboard-main">
        ...
        <div className="applications-stats">
          <div className="app-stat">
            <div className="app-stat__value">{stats.total}</div>
            <div className="app-stat__label">Total</div>
          </div>
          <div className="app-stat app-stat--orange">
            <div className="app-stat__value">{stats.pending}</div>
            <div className="app-stat__label">Pending</div>
          </div>
          <div className="app-stat app-stat--green">
            <div className="app-stat__value">{stats.accepted}</div>
            <div className="app-stat__label">Accepted</div>
          </div>
          <div className="app-stat app-stat--red">
            <div className="app-stat__value">{stats.refused}</div>
            <div className="app-stat__label">Refused</div>
          </div>
        </div>
        ...
        <div className="applications-filters">
          {[
            ...
            ['refused',  `Refused (${stats.refused})`],
          ].map(([key, label]) => (
            <button key={key}
              className={`app-filter ${filter === key ? 'app-filter--active' : ''}`}
        ...
        <div className="applications-list">
          {loading || previewLoading ? (
            ...
            <div className="applications-empty">
              ...
              {filter === 'all' && (
                <Btn variant="primary" onClick={() => navigate('/search')}>
                  Browse Internships
                </Btn>
              )}
            </div>
